        ...
        {isInitializing ? (
          <div className="flex flex-col items-center justify-center py-32 animate-fade-up">
            <div className="w-12 h-12 rounded-xl bg-primary/10 border border-primary/30 flex items-center justify-center animate-pulse-glow mb-4">
              <svg width="20" height="20" viewBox="0 0 16 16" fill="none" className="text-primary animate-spin" style={{ animationDuration: '3s' }}>
          ...
          <div className="mt-8 animate-fade-up">
            <div role="tablist" className="flex items-center gap-1 p-1 rounded-xl bg-secondary/50 border border-border/50 mb-6">
              {tabs.map((tab) => {
            ...
            <div role="tabpanel" id="main-tabpanel" aria-labelledby={`tab-${tabs.find(t => t.to === pathname)?.label.toLowerCase() || "upload"}`}>
              <Outlet />
            </div>
          </div>
        ) : (
          <div className="flex flex-col items-center justify-center py-32 animate-fade-up">
            ...
            <p className="text-muted-foreground mb-8 text-center max-w-md">
              Connect your NEAR wallet to explore the social graph, upload files, and browse on-chain data.
            </p>
            <Alert variant="default" className="max-w-sm border-primary/30 bg-primary/5">
              <AlertDescription className="text-center">
                Click <span className="font-semibold text-primary">Sign In</span> above to connect your wallet
              </AlertDescription>
            </Alert>
          </div>
        )}
